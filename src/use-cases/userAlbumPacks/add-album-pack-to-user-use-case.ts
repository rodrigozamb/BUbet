...
export class AddAlbumPackToUserUseCase {
    ...
    async execute({quantity, user_id, pack_id}: AddAlbumPackToUserUseCaseRequest): Promise<AddAlbumPackToUserUseCaseResponse> {
        const recentCard = await this.userAlbumPacksRepository.checkUserLastAlbumPack(user_id, pack_id)
        if (recentCard) {
            throw new Error("Você precisa esperar até amanhã para comprar mais pacotes deste álbum")
        }
